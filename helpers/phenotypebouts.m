filedir = 'Y:\public\mikey_public\female_preference\tempo_stimuli';
birdnames = getdirs(filedir);
D = [];

for z = 1:length(birdnames)
    bf = fullfile(birdnames(z).folder, birdnames(z).name);
    d = getdirs(bf);
    ...
    if isempty(nsfiles)
        nswavs = dir(fullfile(nsdirpth, '*.wav'));
        for i = 1:length(nswavs)
            WhispSegFile(fullfile(nswavs(i).folder, nswavs(i).name))
        end
    ...
    findata = table('size', [length(variants) 4],...
        'variabletypes',["string", "double", "double", "double"],...
        'variablenames', {'subject', 'change', 'tempo', 'sem'});

    for i = 1:length(variants)
        v = alldata(alldata.change == variants(i),:);
        bts = v.syllables./v.length;

        findata(i,:).subject = birdnames(z).name;
...
sf = fullfile(filedir, 'tempo_stimuli_phenotype.csv');
fprintf('Saving file ...')
writetable(D, sf);
fprintf(' done\n')
